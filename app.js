...
function formatLocalStamp(d){
  // YYYYMMDD_HHmmss (ikut waktu lokal PC)
  return `${d.getFullYear()}${pad2(d.getMonth()+1)}${pad2(d.getDate())}_${pad2(d.getHours())}${pad2(d.getMinutes())}${pad2(d.getSeconds())}`;
}
function sanitizeFileBaseName(name){
  return String(name || "").trim().replace(/[^a-z0-9_\-]+/gi, "_").replace(/^_+|_+$/g, "");
}

const formatSelect= document.getElementById("formatSelect");
const zipToggle   = document.getElementById("zipToggle");
const zipNameWrap = document.getElementById("zipNameWrap");
...
const wbTempRange = document.getElementById("wbTempRange");
const wbTempVal = document.getElementById("wbTempVal");
const wbTintRange = document.getElementById("wbTintRange");
const wbTintVal = document.getElementById("wbTintVal");

const dehazeRange = document.getElementById("dehazeRange");
const dehazeVal = document.getElementById("dehazeVal");
...
// ==========================
const processedCache = new Map(); // key: photoId|s|temp|tint -> canvas
function clamp255(v){ return v < 0 ? 0 : (v > 255 ? 255 : v); }

// Pixel ops ONLY when needed:
// - Shadows lift
// - White balance temp/tint (simple RGB mix)
function getProcessedCanvas(img, photoId, shadows, wbTemp, wbTint){
  const s = Number(shadows || 0);
  const t = Number(wbTemp || 0);
  const k = Number(wbTint || 0);

  const need = (s > 0) || (t !== 0) || (k !== 0);
  if (!need) return null;

  const key = `${photoId}|s${s}|t${t}|k${k}`;
  ...
  // Temp: -50..50 => -1..1
  const temp = Math.max(-50, Math.min(50, t)) / 50;
  const rTemp = 1 + (temp * 0.12);
  const bTemp = 1 - (temp * 0.12);

  // Tint: -50..50 => -1..1
  const tint = Math.max(-50, Math.min(50, k)) / 50;
  const gTint = 1 - (tint * 0.10);
  const rbTint = 1 + (tint * 0.06);
  ...
  for (let i = 0; i < d.length; i += 4){
    ...
    if (sh > 0){
      ...
      const tt = 1 - Math.min(1, lum / 170);
      const lift = (tt * tt) * sh * 110;
...
const c = Number(safe.contrast || 0);   // -50..50
const wbT = Number(safe.wbTemp || 0);   // -50..50
const wbK = Number(safe.wbTint || 0);   // -50..50
const dh = Number(safe.dehaze || 0);    // 0..100

const brightMul = 1 + (b / 50) * 0.30;       // 0.7..1.3
const contrastMul = 1 + (c / 50) * 0.35;     // 0.65..1.35

const dehaze01 = Math.max(0, Math.min(1, dh / 100));
const dehazeContrast = 1 + dehaze01 * 0.25;  // 1..1.25
const dehazeSat = 1 + dehaze01 * 0.20;       // 1..1.20
const dehazeBright = 1 - dehaze01 * 0.05;    // 1..0.95

const photoId = safe._photoId || draftPhotoId || "x";
const processed = getProcessedCanvas(img, photoId, s, wbT, wbK);
const src = processed || img;

let filterStr = "";

const sat = Number(safe.saturation || 0);
const vib = Number(safe.vibrance || 0);

// saturation biasa
const satMul = 1 + (sat / 50) * 0.5; // 0.5x – 1.5x

// vibrance (lebih subtle)
const vibMul = 1 + (vib / 50) * 0.3;

if (sat !== 0) filterStr += ` saturate(${satMul})`;
if (vib !== 0) filterStr += ` saturate(${vibMul})`;
if (b !== 0) filterStr += ` brightness(${brightMul})`;
if (c !== 0) filterStr += ` contrast(${contrastMul})`;
if (dh > 0)  filterStr += ` brightness(${dehazeBright}) contrast(${dehazeContrast}) saturate(${dehazeSat})`;
...
function getDefaultEdit(img){
  ...
  return {
    ...
    contrast: 0,   // -50..50
    wbTemp: 0,     // -50..50 (cool <-> warm)
    wbTint: 0,     // -50..50 (green <-> magenta)
    dehaze: 0,     // 0..100
    saturation: 0,  // -50..50
    vibrance: 0,    // -50..50
...
function syncLightUI(){
  ...
  if (contrastRange) contrastRange.value = String(draftEdit.contrast || 0);
  if (contrastVal) contrastVal.textContent = String(draftEdit.contrast || 0);

  if (wbTempRange) wbTempRange.value = String(draftEdit.wbTemp || 0);
  if (wbTempVal) wbTempVal.textContent = String(draftEdit.wbTemp || 0);

  if (wbTintRange) wbTintRange.value = String(draftEdit.wbTint || 0);
  if (wbTintVal) wbTintVal.textContent = String(draftEdit.wbTint || 0);

  if (dehazeRange) dehazeRange.value = String(draftEdit.dehaze || 0);
  if (dehazeVal) dehazeVal.textContent = String(draftEdit.dehaze || 0);
  
  if (satRange) satRange.value = String(draftEdit.saturation || 0);
  if (satVal) satVal.textContent = String(draftEdit.saturation || 0);
  
  if (vibRange) vibRange.value = String(draftEdit.vibrance || 0);
  if (vibVal) vibVal.textContent = String(draftEdit.vibrance || 0);
...
contrastRange?.addEventListener("input", () => {
  if (!draftEdit) return;
  draftEdit.contrast = Number(contrastRange.value);
  if (contrastVal) contrastVal.textContent = String(draftEdit.contrast);
  redrawEditor();
  refreshPreview();
});

wbTempRange?.addEventListener("input", () => {
  if (!draftEdit) return;
  draftEdit.wbTemp = Number(wbTempRange.value);
  if (wbTempVal) wbTempVal.textContent = String(draftEdit.wbTemp);
  redrawEditor();
  refreshPreview();
});

wbTintRange?.addEventListener("input", () => {
  if (!draftEdit) return;
  draftEdit.wbTint = Number(wbTintRange.value);
  if (wbTintVal) wbTintVal.textContent = String(draftEdit.wbTint);
  redrawEditor();
  refreshPreview();
});

dehazeRange?.addEventListener("input", () => {
  if (!draftEdit) return;
  draftEdit.dehaze = Number(dehazeRange.value);
  if (dehazeVal) dehazeVal.textContent = String(draftEdit.dehaze);
  redrawEditor();
  refreshPreview();
});
...
satRange?.addEventListener("input", () => {
  if (!draftEdit) return;
  draftEdit.saturation = Number(satRange.value);
  if (satVal) satVal.textContent = String(draftEdit.saturation);
  redrawEditor();
  refreshPreview();
});
...
vibRange?.addEventListener("input", () => {
  if (!draftEdit) return;
  draftEdit.vibrance = Number(vibRange.value);
  if (vibVal) vibVal.textContent = String(draftEdit.vibrance);
  redrawEditor();
  refreshPreview();
});
...
exportBtn?.addEventListener("click", async () => {
  ...
  const exportStamp = formatLocalStamp(exportDate);

const zipNameInput = document.getElementById("zipNameInput");
let baseZipName = zipNameInput?.value?.trim() || "";

// default if kosong
if (!baseZipName) baseZipName = "output";

// sanitize: tukar space/simbol pelik kepada underscore (safe untuk Windows/Mac)
baseZipName = baseZipName.replace(/[^a-z0-9_\-]/gi, "_");
  ...
  try {
    ...
    for (let i = 0; i < photoItems.length; i++) {
      ...
      if (zipMode) {
        zip.file(outName, blob, { date: exportDate });
      } else {
    ...
    if (zipMode) {
      ...
      downloadBlob(zipBlob, `${baseZipName}_${exportStamp}.zip`);
    }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function syncZipUI(){
  const on = !!zipToggle?.checked;
  if (zipNameWrap) zipNameWrap.classList.toggle("isHidden", !on);

  if (on && zipNameInput){
    // kalau kosong, auto letak default (senang user)
    const cur = zipNameInput.value.trim();
    if (!cur){
      zipNameInput.value = "output";
    }
  }
}
...
zipToggle?.addEventListener("change", () => {
  ...
  syncZipUI(); // ✅ terus update tanpa refresh
});
